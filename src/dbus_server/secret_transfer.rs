...
use aes::{cipher::{block_padding::Pkcs7, consts::U16, generic_array::GenericArray, BlockDecryptMut, BlockEncryptMut, KeyIvInit}, Aes128};
use hkdf::Hkdf;
...
impl DhIetf1024Sha256Aes128CbcPkcs7Transfer {
    pub fn new(client_pub_key: &[u8]) -> Result<Self> {
        ...
        let mut okm = [0; 16];
        // empty info
        hk.expand(&[], &mut okm).map_err(|_| Error::EncryptionError("Invalid length"))?;
    ...
    pub fn get_pub_key(&self) -> Vec<u8> {
        // 2 ^ priv_key % dh_prime
        (&*TWO).modpow(&self.server_priv, &*DH_SECOND_OAKLEY_PRIME).to_bytes_be()
    }
}
