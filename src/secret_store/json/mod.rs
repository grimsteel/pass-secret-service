...
use crate::{
    error::{OptionNoneNotFound, Result},
    pass::PasswordStore,
    secret_store::{
        json::{
            json_file::JsonFile,
        ...
        slugify, SecretStore, NANOID_ALPHABET, PASS_SUBDIR,
    },
};
...
impl<'a> SecretStore<'a> for JsonSecretStore<'a> {
    async fn new(pass: &'a PasswordStore) -> Result<Self> {
            ...
            .filter(|(file_type, _)| file_type.is_dir())
        {
            ...
            let mut file = pass.open_file(collection_settings_path).await?;
            let settings = <Collection as JsonFile<_>>::read(&mut file).await?;
            ...
            {
                let item_settings_path =
                    collection_dir.join(secret_filename.with_extension("json"));

                let mut item_file = pass.open_file(item_settings_path).await?;
                let item_settings = <Item as JsonFile<_>>::read(&mut item_file).await?;

                // remove extension for normal id
                let item_id = secret_filename
                    .file_stem()
    ...
    async fn list_all_aliases(&self) -> Result<HashMap<String, Vec<String>>> {
        ...
        Ok(collections
            .iter()
            .map(|(id, coll)| (id.clone(), coll.settings.aliases.iter().cloned().collect()))
            .collect())
    }
    ...
    async fn set_alias(&self, alias: Arc<String>, collection: Option<String>) -> Result<()> {
        ...
        for (id, col) in collections.iter_mut() {
            ...
            // the save = !save business is so that we don't save if we set the collection which was already set
    ...
    async fn create_collection(
        ...
        if let Some(alias) = &alias {
            if let Some((id, existing_collection)) = collections.iter_mut().find(|c| c.0 == alias) {
                // update label if provided
                if let Some(label) = label {
                    existing_collection.settings.label = label;
                }
        ...
        if let Some(alias) = alias {
            for (_, col) in collections.iter_mut().filter(|c| c.0 == &alias) {
        ...
        let collection_dir = Collection::collection_dir(&id);
        self.pass.make_dir(&collection_dir).await?;

        // Write collection settings file
        let collection_settings_path = collection_dir.join(COLLECTION_SETTINGS_FILE);
        let mut collection = Collection {
            ...
            file: self.pass.open_file(&collection_settings_path).await?,
            items: HashMap::new(),
        };
        collection.save_settings().await?;
    ...
    async fn delete_collection(&self, collection_id: Arc<String>) -> Result {
        ...
        self.collections
            .lock()
            .await
            .remove(collection_id.as_ref())
            .into_not_found()?;
        // delete collection dir
        self.pass
            .remove_dir(Collection::collection_dir(collection_id.as_ref()))
            .await?;
    ...
    async fn stat_secret(&self, collection_id: &str, secret_id: &str) -> Result<Metadata> {
        let secret_path =
            Collection::collection_dir(collection_id).join(format!("{}.gpg", secret_id));
    ...
    async fn create_secret(
        ...
        let collection_dir = Collection::collection_dir(&collection_id);
        ...
        // Write item settings
        let item_settings_path = collection_dir.join(format!("{}.json", secret_id));
        let mut item = Item {
            settings: ItemSettings {
                label: label.unwrap_or_else(|| "Untitled Secret".into()),
                attrs: attributes.as_ref().clone(),
            },
            file: self.pass.open_file(&item_settings_path).await?,
        };

        item.save_settings().await?;

        // Update in-memory collection
        collection.items.insert(secret_id.clone(), item);
    ...
    async fn set_secret_label(
        ...
        let mut collections = self.collections.lock().await;
        let collection_obj = collections.get_mut(collection.as_ref()).into_not_found()?;

        let item = collection_obj
            .items
    ...
    async fn set_secret_attrs(
        ...
        let item = collection
            .items
            .get_mut(secret_id.as_ref())
            .into_not_found()?;
        item.settings.attrs = attrs;
        item.save_settings().await?;
